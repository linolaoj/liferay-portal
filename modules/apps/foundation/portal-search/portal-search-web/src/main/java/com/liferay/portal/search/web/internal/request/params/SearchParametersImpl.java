...
public class SearchParametersImpl implements SearchParameters {
	...
	@Override
	public Optional<Integer> getFromParameterValue() {
		Optional<String> parameterOptional = getParameter(
			_configuration.getFromParameterName());

		return parameterOptional.map(GetterUtil::getInteger);
	}

	@Override
	public Optional<String> getKeywordsParameterValue() {
		return getParameter(_configuration.getKeywordsParameterName());
	}

	protected Optional<String> getParameter(String name) {
		return StringUtil.maybe(ParamUtil.getString(_request, name));
	}
