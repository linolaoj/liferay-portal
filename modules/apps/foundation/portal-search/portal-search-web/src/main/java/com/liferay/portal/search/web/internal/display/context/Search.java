...
import com.liferay.portal.search.web.internal.search.SearchSettingsImpl;
...
public class Search {
	...
	protected SearchContainer<Document> buildSearchContainer(
		...
		SearchContainer<Document> searchContainer =
			_searchContainerSupplier.getSearchContainer(
				searchSettingsImpl.getStartPageParamName(),
				searchSettingsImpl.getStartPage());
	...
	protected void contribute(SearchSettings searchSettings) {
		...
		searchContributors.forEach(
			searchContributor -> searchContributor.contribute(searchSettings));
	}
	...
	protected Hits search(SearchContext searchContext) {
		Optional<String> keywordsOptional = StringUtil.maybe(
