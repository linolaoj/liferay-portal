 ...
 * @author Lino Alves
 */
public class AssetEntriesSearchFacetDisplayBuilder 
	...
	public AssetEntriesSearchFacetDisplayContext build() {
		...
		assetEntriesSearchFacetDisplayContext.setParamValue(_paramValue);
	...
	public AssetEntriesSearchFacetTermDisplayContext buildTermDisplay(
		String typeName, boolean selected, String assetType,
		int frequency) {
	...
	public List<AssetEntriesSearchFacetTermDisplayContext> buildTermDisplayContexts() {
		...
		List<String> assetTypes = 
			new SortedArrayList<String>(new ModelResourceComparator(_locale));

		for (String className : _classNames) {
			if (assetTypes.contains(className) || !ArrayUtil.contains(_classNames, className)) {
				continue;
			}
		...
		for (String assetType : assetTypes) {
			...
			AssetRendererFactory<?> assetRendererFactory = 
					AssetRendererFactoryRegistryUtil.getAssetRendererFactoryByClassName(assetType);
			...
			AssetEntriesSearchFacetTermDisplayContext
				assetEntriesSearchFacetFieldDisplayContext = buildTermDisplay(
					...
					selected, assetType, termCollector.getFrequency());
	...
	public boolean isNothingSelected() {
		return Validator.isNull(_paramValue);
	}
