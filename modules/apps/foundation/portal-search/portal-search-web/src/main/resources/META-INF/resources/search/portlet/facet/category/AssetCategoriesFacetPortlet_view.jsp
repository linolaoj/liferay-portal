...
AssetCategoriesSearchFacetDisplayContext assetCategoriesSearchFacetDisplayContext =
	(AssetCategoriesSearchFacetDisplayContext)java.util.Objects.requireNonNull(
		request.getAttribute(AssetCategoriesSearchFacetDisplayContext.ATTRIBUTE));
...
<c:choose>
	...
	<c:otherwise>
		<liferay-ui:panel-container extended="true" id='<%= namespace + "facetAssetCategoriesPanelContainer" %>' markupView="lexicon" persistState="true">
			<liferay-ui:panel collapsible="true" cssClass="search-facet" id='<%= namespace + "facetAssetCategoriesPanel" %>' markupView="lexicon" persistState="true" title="sites">
				<aui:form method="post" name="categoryFacetForm">
					<aui:input autocomplete="off" name="<%= HtmlUtil.escapeAttribute(assetCategoriesSearchFacetDisplayContext.getParamName()) %>" type="hidden" value="<%= assetCategoriesSearchFacetDisplayContext.getParamValue() %>" />

					<c:if test="<%= !assetCategoriesSearchFacetDisplayContext.isNothingSelected() %>">
						<aui:a cssClass="text-default" href="javascript:;" onClick='<%= namespace + "_clearFacet('" + assetCategoriesSearchFacetDisplayContext.getParamName() + "');" %>'><liferay-ui:message key="facet-portlet-clear" /></aui:a>
					</c:if>
									...
									<label class="facet-checkbox-label" for="<portlet:namespace /><%= termName %>">
										...
										<c:if test="<%= assetCategoriesSearchFacetTermDisplayContext.isFrequencyVisible() %>">
											<small class="term-count">
												<liferay-ui:message arguments="<%= assetCategoriesSearchFacetTermDisplayContext.getFrequency() %>" key="(x)" />
											</small>
										</c:if>
									</label>
								</li>
