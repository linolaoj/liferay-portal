...
<c:choose>
	...
	<c:otherwise>
		<liferay-ui:panel-container extended="true" id='<%= renderResponse.getNamespace() + "facetAssetEntriesPanelContainer" %>' markupView="lexicon" persistState="true">
			<liferay-ui:panel collapsible="true" cssClass="search-facet" id='<%= renderResponse.getNamespace() + "facetAssetEntriesPanel" %>' markupView="lexicon" persistState="true" title="portlet.type-facet.title">
				<aui:form method="post" name="assetEntriesFacetForm">
					...
					<c:if test="<%= !assetEntriesSearchFacetDisplayContext.isNothingSelected() %>">
						<aui:a cssClass="text-default" href="javascript:;" onClick='<%= renderResponse.getNamespace() + "_clearFacet('" + assetEntriesSearchFacetDisplayContext.getParamName() + "');" %>'><small><liferay-ui:message key="portlet.type-facet.clear" /></small></aui:a>
					</c:if>
				</aui:form>
			</liferay-ui:panel>
