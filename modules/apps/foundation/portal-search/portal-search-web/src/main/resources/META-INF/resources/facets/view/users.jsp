...
<%
com.liferay.portal.search.web.internal.facet.display.context.UserSearchFacetDisplayContext userSearchFacetDisplayContext = new com.liferay.portal.search.web.internal.facet.display.context.UserSearchFacetDisplayContext(facet, fieldParam, dataJSONObject.getInt("frequencyThreshold"), dataJSONObject.getInt("maxTerms", 10), dataJSONObject.getBoolean("showAssetCount", true));
%>

<c:choose>
	<c:when test="<%= userSearchFacetDisplayContext.isRenderNothing() %>">
		<aui:input autocomplete="off" name="<%= HtmlUtil.escapeAttribute(userSearchFacetDisplayContext.getFieldParamInputName()) %>" type="hidden" value="<%= userSearchFacetDisplayContext.getFieldParamInputValue() %>" />
	</c:when>
	<c:otherwise>
			...
			<div class="panel-body">
				<div class="<%= cssClass %>" data-facetFieldName="<%= HtmlUtil.escapeAttribute(facet.getFieldId()) %>" id="<%= randomNamespace %>facet">
					...
					<ul class="list-unstyled users">
						...
						<%
						List<com.liferay.portal.search.web.internal.facet.display.context.UserSearchFacetTermDisplayContext> userSearchFacetTermDisplayContexts =
						...
						%>
							...
							<li class="facet-value">
								<a class="<%= userSearchFacetTermDisplayContext.isSelected() ? "text-primary" : "text-default" %>" data-value="<%= HtmlUtil.escapeAttribute(userSearchFacetTermDisplayContext.getUserName()) %>" href="javascript:;">
									...
									<c:if test="<%= userSearchFacetTermDisplayContext.isShowFrequency() %>">
