...
String paramName = siteFacetPortletDisplayContext.getFieldParamInputName(); 
%>
			...
			<liferay-ui:panel collapsible="true" cssClass="search-facet" id='<%= namespace + "facetScopePanel" %>' markupView="lexicon" persistState="true" title="sites">
				...
				%>
					<aui:input autocomplete="off"
						...
						type="hidden"
						value="<%= siteFacetPortletTermDisplayContext.getValue() %>"
					/>
				
					<aui:input 
						checked="<%= siteFacetPortletTermDisplayContext.isSelected() %>" 
						cssClass="<%= cssClassFacetTerm %>" 
						label='TODO inline above!' 
						name="<%= termName %>" 
						type="checkbox">
					
						<span style="font-size: small; color: black;"><%= HtmlUtil.escape(siteFacetPortletTermDisplayContext.getTerm()) %></span>
						<c:if test="<%= siteFacetPortletTermDisplayContext.isFrequencyVisible() %>">
							<span style="font-size: x-small; color: gray;">&nbsp;(<%= siteFacetPortletTermDisplayContext.getFrequency() %>)</span>
						</c:if>
