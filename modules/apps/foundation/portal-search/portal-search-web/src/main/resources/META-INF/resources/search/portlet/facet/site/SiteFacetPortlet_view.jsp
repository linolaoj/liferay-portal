			...
			<liferay-ui:panel collapsible="true" cssClass="search-facet" id='<%= namespace + "facetScopePanel" %>' markupView="lexicon" persistState="true" title="sites">
				<aui:form method="post" name="siteFacetForm">
					...
					<c:if test="<%= !siteFacetPortletDisplayContext.isNothingSelected() %>">
						<aui:a cssClass="text-default" href="javascript:;" onClick='<%= namespace + "_clearFacet('" + siteFacetPortletDisplayContext.getFieldParamInputName() + "');" %>'><liferay-ui:message key="facet-portlet-clear" /></aui:a>
					</c:if>
							...
							<label class="facet-checkbox-label" for="<portlet:namespace /><%= termName %>">
								...
								<c:if test="<%= siteFacetPortletTermDisplayContext.isFrequencyVisible() %>">
									<small class="term-count">
										<liferay-ui:message arguments="<%= siteFacetPortletTermDisplayContext.getFrequency() %>" key="(x)" />
									</small>
								</c:if>
							</label>
