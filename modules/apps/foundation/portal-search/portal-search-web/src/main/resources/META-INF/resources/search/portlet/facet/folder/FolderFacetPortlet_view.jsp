...
<c:choose>
	...
	<c:otherwise>
		<liferay-ui:panel-container extended="true" id='<%= renderResponse.getNamespace() + "facetFoldersPanelContainer" %>' markupView="lexicon" persistState="true">
			<liferay-ui:panel collapsible="true" cssClass="search-facet" id='<%= renderResponse.getNamespace() + "facetFoldersPanel" %>' markupView="lexicon" persistState="true" title="portlet.folder-facet.title">
				<aui:form method="post" name="FoldersFacetForm">
					...
					<c:if test="<%= !folderSearchFacetDisplayContext.isNothingSelected() %>">
						<aui:a cssClass="text-default" href="javascript:;" onClick='<%= renderResponse.getNamespace() + "_clearFacet('" + folderSearchFacetDisplayContext.getParamName() + "');" %>'>
							<small><liferay-ui:message key="portlet.folder-facet.clear" /></small>
						</aui:a>
					</c:if>
				</aui:form>
			</liferay-ui:panel>
