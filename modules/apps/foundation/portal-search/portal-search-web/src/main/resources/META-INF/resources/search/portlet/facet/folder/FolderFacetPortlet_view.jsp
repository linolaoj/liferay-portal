...
FolderSearchFacetDisplayContext folderSearchFacetDisplayContext =
	(FolderSearchFacetDisplayContext)java.util.Objects.requireNonNull(
		request.getAttribute(FolderSearchFacetDisplayContext.ATTRIBUTE));
...
<c:choose>
	...
	<c:otherwise>
		<liferay-ui:panel-container extended="true" id='<%= namespace + "facetFoldersPanelContainer" %>' markupView="lexicon" persistState="true">
			<liferay-ui:panel collapsible="true" cssClass="search-facet" id='<%= namespace + "facetFoldersPanel" %>' markupView="lexicon" persistState="true" title="folders">
				<aui:form method="post" name="FoldersFacetForm">
					<aui:input autocomplete="off" name="<%= HtmlUtil.escapeAttribute(folderSearchFacetDisplayContext.getParamName()) %>" type="hidden" value="<%= folderSearchFacetDisplayContext.getParamValue() %>" />

					<c:if test="<%= !folderSearchFacetDisplayContext.isNothingSelected() %>">
						<aui:a cssClass="text-default" href="javascript:;" onClick='<%= namespace + "_clearFacet('" + folderSearchFacetDisplayContext.getParamName() + "');" %>'><liferay-ui:message key="facet-portlet-clear" /></aui:a>
					</c:if>
									...
									<label class="facet-checkbox-label" for="<portlet:namespace /><%= termName %>">
										...
										<c:if test="<%= folderSearchFacetTermDisplayContext.isFrequencyVisible() %>">
											<small class="term-count">
												<liferay-ui:message arguments="<%= folderSearchFacetTermDisplayContext.getFrequency() %>" key="(x)" />
											</small>
										</c:if>
									</label>
								</li>
