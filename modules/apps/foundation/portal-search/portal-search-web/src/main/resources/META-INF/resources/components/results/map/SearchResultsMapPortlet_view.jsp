...
>
	...
	>
		...
		<%
		com.liferay.portal.search.web.internal.result.display.context.SearchResultSummaryDisplayContext searchResultSummaryDisplayContext = searchResultsMapDisplayContext.getSummary(document);
		%>
			...
			<c:if test="<%= searchResultSummaryDisplayContext.isAssetIconVisible() %>">
				<span class="search-asset-type-sticker sticker sticker-default sticker-lg sticker-rounded sticker-static">
					<svg class="lexicon-icon">
						<use xlink:href="<%= themeDisplay.getPathThemeImages() %>/lexicon/icons.svg#<%= searchResultSummaryDisplayContext.getAssetIcon() %>" />
					</svg>
				</span>
			</c:if>
		</liferay-ui:search-container-column-text>
